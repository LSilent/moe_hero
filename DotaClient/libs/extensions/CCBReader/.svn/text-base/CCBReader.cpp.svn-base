...
CCNode* CCBReader::readNodeGraphFromFile(const char *pCCBFileName, CCObject *pOwner, const CCSize &parentSize)
{
	...
	bool biphone5 =false;
	if( (parentSize.width == 640 && parentSize.height == 1136) || (parentSize.width == 1136 && parentSize.height == 640) )
	{
		biphone5 =true;
	}
	bool bipad = false;
	if (!biphone5)
	{
		if( (parentSize.width == 1024 && parentSize.height == 768) || (parentSize.width == 768 && parentSize.height == 1024) )
			bipad =true;
		if( (parentSize.width == 2048 && parentSize.height == 1536) || (parentSize.width == 1536 && parentSize.height == 2048) )
			bipad =true;
	}
	...
	}else
	{
		if(biphone5)
		{
			strCCBFileName = strCCBFileName.substr(0,strCCBFileName.length() - strSuffix.length());
			strCCBFileName+= "-ip5";
			strCCBFileName += strSuffix;
		}
		if (bipad)
